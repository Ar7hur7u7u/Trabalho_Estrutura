...
public class ExperimentoExclusao {
    ...
    public ResultadoExclusao executar() {
        ResultadoExclusao resultado = new ResultadoExclusao();
    ...
    public static class ResultadoExclusao {
        private final double[][] medias = new double[3][2];
        private final double[][] desvios = new double[3][2];

        public void imprimirTabela() {
            String[] cenarios = {
                "Exclusão c/ carga crescente",
                "Exclusão c/ carga decrescente",
                "Exclusão c/ carga aleatória"
            };

            System.out.println("\n===== RESULTADOS DE EXCLUSÃO (ns) =====");
            System.out.printf("%-32s %-24s %-24s%n", "Cenário", "Estrutura Crescente", "Estrutura Decrescente");
            for (int i = 0; i < cenarios.length; i++) {
                ...
                System.out.printf("%-32s %-24s %-24s%n", cenarios[i], colCresc, colDecresc);
            }
        }
    }
}
